...
test("D1 creates an activity without a public invitation", async () => {
  const database = setup();
  const store = new D1ActivityStore(database as unknown as D1Database);
  const publicInput = { ...input };
  delete publicInput.invitationCode;
  const tokens = ["public-activity", "public-organizer", "public-host", "public-event", "public-display", "public-report"];
  ...
  assert.equal((await resolveCapability("public-event", "participant", store)).activity.id, result.activity.id);
  assert.equal(
    database.sqlite.prepare("SELECT status FROM invitations WHERE code = ?").get("activity:public-activity")?.status,
    "used",
  );
});
